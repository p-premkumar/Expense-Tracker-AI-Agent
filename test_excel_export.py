...
try:
    ...
    print(f"❌ Failed to import ExcelExporter: {e}")
    sys.exit(1)
...
try:
    ...
    print("✅ ExpenseDatabase imported successfully")
except ImportError as e:
    print(f"❌ Failed to import ExpenseDatabase: {e}")
    sys.exit(1)

try:
    from bot_commands import export_all, export_monthly, export_weekly, export_today_data
    print("✅ All export commands imported successfully")
except ImportError as e:
    print(f"❌ Failed to import export commands: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"❌ Failed to initialize ExcelExporter: {e}")
    sys.exit(1)
...
try:
    ...
    if expenses:
        print(f"✅ Test expense retrieved: {len(expenses)} expense(s) found")
...
try:
    ...
    if os.path.exists(file1):
        print(f"✅ All expenses export created: {file1}")
        os.remove(file1)
    ...
    if os.path.exists(file2):
        print(f"✅ Monthly export created: {file2}")
        os.remove(file2)
        print("✅ File cleaned up")
    else:
    ...
    if os.path.exists(file3):
        print(f"✅ 7-day export created: {file3}")
        os.remove(file3)
        print("✅ File cleaned up")
    else:
    ...
    if os.path.exists(file4):
        print(f"✅ Today's export created: {file4}")
        os.remove(file4)
        print("✅ File cleaned up")
    else:
